...
# Define x and y
x<-seq(1,length(rmsf[,11]))
y<-cbind(rmsf[,11])

x2<-seq(1,length(rmsf2[,11]))
y2<-cbind(rmsf2[,11])

# Draw Lines
ggplot()+
geom_point(data=rmsf,aes(x,y,color="red"),    size=2.0)  + 
geom_point(data=rmsf2,aes(x2,y2,color="pink"), size=2.0)  + 
geom_line(data=rmsf,aes(x,y,color="red"),    size=1.5)  + 
...
# labs title
labs(title="",x="",y="HB acceptors") +

# legend
scale_color_manual(name="",values=c("red","pink"),label=c(name,name2)) +

#scale
scale_y_continuous(limits=c(-6,5),breaks=seq(-6,5,2),label=seq(-6,5,2))  +
...
axis.title.y=element_text(size=12,face="bold",color="red"),
axis.text.x=element_text(size=10,face="bold"),
axis.text.y=element_text(size=10,face="bold",color="red")
) 



# Save png
ggsave("./HBacceptors.png",width=18,height=6,dpi=300)
