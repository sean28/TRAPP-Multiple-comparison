...
# Define x and y
x<-seq(1,length(rmsf[,8]))
y<-cbind(rmsf[,8])

x2<-seq(1,length(rmsf2[,8]))
y2<-cbind(rmsf2[,8])

# Draw Lines
ggplot()+
geom_point(data=rmsf,aes(x,y,color="darkgreen"),    size=2.0)  + 
geom_point(data=rmsf2,aes(x2,y2,color="yellowgreen"), size=2.0)  + 
geom_line(data=rmsf,aes(x,y,color="darkgreen"),    size=1.5)  + 
geom_line(data=rmsf2,aes(x2,y2,color="yellowgreen"), size=1.5)  + 
...
# labs title
labs(title="",x="",y="Pos. charged") +

# legend
scale_color_manual(name="",values=c("darkgreen","yellowgreen"),label=c(name,name2)) +

#scale
scale_y_continuous(limits=c(-2,2),breaks=seq(-2,2,1),label=seq(-2,2,1))  +
...
axis.title.y=element_text(size=12,face="bold",color="darkgreen"),
axis.text.x=element_text(size=10,face="bold"),
axis.text.y=element_text(size=10,face="bold",color="darkgreen")
) 


# Save png
ggsave("./Pos.charged.png",width=18,height=6,dpi=300)
